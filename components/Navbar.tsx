...
export default function Navbar({ currentUser }: { currentUser: User | null }) {
  ...
  const fullname: string[] =
    currentUser !== null ? currentUser.name.split(" ") : [];
  ...
  return (
    <div className='w-full sticky top-0'>
      <div className='w-full p-2 bg-white dark:bg-neutral-800 shadow-2xl dark:shadow-neutral-700 h-[60px] flex flex-row justify-between items-center gap-2'>
        ...
        <div
          className='hidden md:flex flex-row gap-1 items-center dark:text-white text-black'
          onClick={() => router.push("/")}
        ></div>
        <div
          className='flex md:hidden flex-row gap-1 items-center dark:text-white text-black'
          onClick={() => router.push("/")}
        >
          <span className='w-5 h-5 outline-none md:text-lg text-sm dark:border-white border-black rounded-full border-[1px] border-solid flex justify-center items-center text-center'>
            ?
          </span>
        ...
        <div className='hidden md:flex flex-row justify-center items-center w-[50%]'>
          <input
            ...
            className='p-2 h-[40px] rounded-l-md w-full dark:bg-neutral-700 bg-neutral-300 focus:outline-none'
            placeholder='Search'
            value={search}
            onKeyDown={(e) => {
              if (e.key === "Enter") {
          ...
          />
          <button
            className='dark:bg-neutral-700 bg-neutral-300 rounded-r-md h-[40px] p-2'
            onClick={() => saech()}
          >
            <Search size={18} />
          </button>
              ...
              <TooltipProvider>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button
                      onClick={() => onOpen("addQuestion", { currentUser })}
                      variant={"link"}
                      className='m-1 p-1'
                    >
                      <Plus size={18} />
      ...
      <div
        className={` md:hidden fixed top-0 left-0 w-[90%] transition-all ease-in-out duration-500 ${
