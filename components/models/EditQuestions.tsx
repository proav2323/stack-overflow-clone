...
export default function EditQuestionModel() {
  ...
  const { currentUser, question } = data;

  if (currentUser && question && currentUser.id !== question?.askedBy.id) {
    onClose();
  }

  const [tag, setTag] = useState("");
  const [tags, setTags] = useState<string[]>(question ? question.tags : []);
  const [loading, setLoading] = useState(false);
  const open = isOpen && type === "editQuestion";
  const form = useForm<z.infer<typeof addQuestionSchema>>({
    resolver: zodResolver(addQuestionSchema),
    defaultValues: {
      title: question?.title,
      description: question ? question.description : "",
      expectation: question ? question.expection : "",
    },
  });
  ...
  useEffect(() => {
    if (question) {
      ...
      form.setValue("expectation", question.expection);
  ...
  const onSubmit = (values: z.infer<typeof addQuestionSchema>) => {
    if (tags.length === 0 || !question) {
      toast.error("at least one tag");
      return;
    }
    setLoading(true);
    axios
      .put(`/api/questions/${question.id}`, { ...values, tags: tags })
      .then((data) => {
      ...
      <DialogContent className='gap-2 flex-col flex w-full max-h-[100vh] overflow-y-scroll noScroll'>
        ...
        <Form {...form}>
          <form
            ...
            className='flex-col flex space-y-9'
          >
            <FormField
            ...
            <div className='flex flex-col gap-2 w-full'>
              ...
              />
              {exp === "" && (
                <span className='text-red-500'>Expactation is required</span>
              )}
            </div>
