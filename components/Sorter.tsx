...
export default function Sorter({url, un = true}: {url: string, un?: boolean}) {

  const searchParams = useSearchParams();
  const values = un ? ["new", "oldest", "unanswered"] : ["new", "oldest"]
  ...
  return (
    <div className='flex flex-row justify-start overflow-x-scroll py-2 px-2 items-center gap-2 noScroll max-w-[100%]'>
       {values.map((data) => (
