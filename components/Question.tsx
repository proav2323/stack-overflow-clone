...
export default function Question({
  ...
  const addComment = () => {
    ...
    setLoading(true);
    axios
      .post(`/api/questions/${question.id}/comment/`, { text: comment })
      .then((data) => {
  ...
  const addAnswer = () => {
    ...
    setLoading(true);
    axios
      .post(`/api/questions/${question.id}/answers/`, { text: answer })
      .then((data) => {
    ...
    <div className='flex flex-col gap-2 w-full justify-start items-center'>
      <Card className='w-full'>
        <CardContent className='flex flex-col justify-start items-start gap-0 px-0 py-0 w-full rouded-t-md'>
          <div className='flex flex-col justify-start items-start gap-2 w-full rounded-t-md bg-orange-700 px-2 py-2 relative'>
            <span className='md:text-lg text-sm font-bold w-[98%] text-left text-white mb-2 mt-1'>
              {question.title}
            </span>
            ...
            <div className='flex justify-between items-center w-full gap-2'>
              <div className='flex flex-row justify-center items-center gap-2'>
                <ProfiEImage
                  fullname={fullName}
                  currentUser={question.askedBy}
                />
                <span className='dark:text-neutral-700 text-neutral-300'>
                  {question.askedBy.name}
                </span>
              </div>
              <span>
                {question.createdAt.toTimeString() ===
                question.updatedAT.toTimeString()
                  ? "Asked"
                  : "Modified"}{" "}
                {timeAgo(question.updatedAT)}
              </span>
            </div>
            <div className='flex flex-row justify-center items-center gap-2 absolute top-2 right-2'>
              {currentUser !== null &&
                question.askedById === currentUser.id && (
                  <PenIcon
                    onClick={() =>
                      onOpen("editQuestion", {
                        question: question,
          ...
          <div className='flex flex-col justify-start items-start gap-2 rounded-b-md px-2 py-2 dark:bg-neutral-900 bg-white w-full'>
            <Editer
              ...
              classN='bg-transparent shadow-none w-full dark:text-white text-black outline-none border-none'
              setValue={undefined}
              id={undefined}
            />
            <Editer
              edit={false}
              value={question.expection}
              classN='bg-transparent shadow-none w-full dark:text-white text-black outline-none border-none'
              setValue={undefined}
              id={undefined}
            />
              ...
              <div className='flex flex-row gap-2 justify-center items-center'>
                <Dialog>
                  ...
                  <DialogContent className='px-2 py-2 flex flex-col justify-center items-center gap-2 max-h-[100%] overflow-y-scroll noScroll '>
                    ...
                    <Editer
                      edit={loading === false ? true : false}
                      id='description'
                      value={answer}
                      setValue={(id, value) => setAnswer(value)}
                    />
                    <Button
                      className='w-full'
                      disabled={loading}
            ...
            <div className='flex flex-col gap-2 justify-start items-start w-full'>
              {question.comment.map((comment) => (
                <CommentCard
                  url={`/api/questions/${question.id}/comment/${comment.id}/`}
      ...
      <div className='flex md:flex-row flex-col justify-between items-center gap-2 w-full'>
        <span className='text-lg font-bold'>
          {String(question.answers.length)} answers
        </span>
